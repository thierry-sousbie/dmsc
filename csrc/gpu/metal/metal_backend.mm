...
void launch_simplify_arcs_metal(torch::Tensor d_cancels, torch::Tensor d_init_t, torch::Tensor d_parent_ptrs,
  ...
  @autoreleasepool {
    ...
    while (iteration < num_cancels) {
      ...
      [contractEncoder setBuffer:getMTLBufferStorage(d_cancels) offset:0 atIndex:1];
      [contractEncoder setBuffer:getMTLBufferStorage(d_init_t) offset:0 atIndex:2];
      [contractEncoder setBuffer:getMTLBufferStorage(d_base_lens) offset:0 atIndex:3];
      [contractEncoder setBuffer:getMTLBufferStorage(d_parent_ptrs) offset:0 atIndex:4];
      [contractEncoder setBuffer:getMTLBufferStorage(d_weights) offset:0 atIndex:5];
      [contractEncoder setBuffer:getMTLBufferStorage(d_dag) offset:0 atIndex:6];
      [contractEncoder setBuffer:mtl_dag_sz offset:0 atIndex:7];
