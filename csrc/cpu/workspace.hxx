...
struct Workspace {
  ...
  DMSComplex complex(bool return_gradient, bool trace_max_arcs, bool trace_min_arcs, bool trace_face_groups,
    ...
    auto t_ridge_faces = trace_max_arcs ? hlp.out_ridge_faces.get().clone() : torch::empty({0}, opts_i);
    auto t_ridge_faces_off = trace_max_arcs ? hlp.out_ridge_faces_off.get().clone() : torch::empty({0}, opts_i);
    auto t_arc_faces_off = trace_max_arcs ? hlp.out_arc_faces_off.get().clone() : torch::empty({0}, opts_i);

    auto t_ridge_vertices = trace_min_arcs ? hlp.out_ridge_vertices.get().clone() : torch::empty({0}, opts_i);
    auto t_ridge_vertices_off = trace_min_arcs ? hlp.out_ridge_vertices_off.get().clone() : torch::empty({0}, opts_i);
    auto t_arc_vertices_off = trace_min_arcs ? hlp.out_arc_vertices_off.get().clone() : torch::empty({0}, opts_i);

    auto t_face_groups = trace_face_groups ? cell_groups.face_groups.get().clone() : torch::empty({0, 0}, opts_i);
    auto t_vertex_groups = trace_vertex_groups ? cell_groups.vertex_groups.get().clone() : torch::empty({0, 0}, opts_i);
